	...
	void Start () {
      ...
      //  player = videoObj.GetComponent<VideoPlayer>();
       ...
       GetComponent<EvidenceDetecting>().exitDrawing += Activate;
        ...
        SetTask("Task: Talk to Sherlock.","WASD to walk around.\nSpace to Jump.\nShift to Run.\nUse Left Click to interact.");
    }
    ...
    void CheckVarUpdate(string s,bool b)
    {
        ...
        switch (s){
            ...
            case "TellSherlockName":
                SetTask("Task: Read the letter and tell Sherlock who the sender is.","You can drag and drop text into your evidences.");
                break;

            case "TellSherlockRelation":
                SetTask("Task: Find the relation between the violin and the letter.","Use P to make drawings of the enviroment.");
                break;
                //case "ReadInvitation":
                //    if (b)
