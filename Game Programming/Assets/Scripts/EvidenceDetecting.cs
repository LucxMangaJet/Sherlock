...
public class EvidenceDetecting : MonoBehaviour
{
	...
	void Update(){
		if (Input.GetKeyDown (KeyCode.P)&&usable) {
		...
		if (inSelection && Input.GetMouseButtonDown (0)) {
			HandleSelection ();
		}
	}
...
//		Debug.Log ("Visible: " + evidences.Count);
	...
	public void OperateButton(bool save ){
		if (save) {
			Texture2D saveText = Instantiate (latestDrawingModified.sprite.texture);
			Sprite saveSprite = Sprite.Create (saveText, latestDrawing.sprite.rect, latestDrawing.sprite.pivot);
			GetComponent<MenuHandler> ().AddDrawing (new Drawing (saveSprite, latestDrawingModified.evidences));
            GetComponent<MenuHandler>().RefitDrawingsContent();
